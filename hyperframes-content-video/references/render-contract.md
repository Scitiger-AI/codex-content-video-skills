...
The composition must have fixed manifest dimensions and a finite, seekable timeline. Derive root duration from audio duration, allowing only a minimal ending tail. Keep visible captions in the resolved caption box and make each visual beat an explanatory visual, not a transcript card. Before every delivery render, run `content-video-workflow/scripts/check_delivery_guides.py --src <project_dir> --caption-layout <caption-layout.json>`. It rejects a narrow, visible, top-left bar whose height equals `visual_content.bottom_y`, preventing a safe-zone boundary from leaking into the video. Inspect safe-zone overlay snapshots automatically, but render guides only in debug stills or preview mode and never in the delivery composition. Rerender after correcting ordinary text, caption, or layout defects.
