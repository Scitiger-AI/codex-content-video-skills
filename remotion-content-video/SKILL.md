...
7. Before rendering, run the JSX text-escape check and the shared delivery-guide check. The latter rejects a visible left-edge bar extending to `visual_content.bottom_y`, which is a leaked safe-zone guide. Render a still from every visual beat plus the longest subtitle segment and inspect them autonomously with the platform UI exclusion and caption box overlaid. Render guides only in a dedicated debug still or preview composition; never mount an overlay, ruler, boundary line, or safe-zone marker in the delivery composition. When either check fails, remove or rework the guide, then rerender; do not request user approval for normal quality fixes.
...
python3 "${CODEX_HOME:-$HOME/.codex}/skills/content-video-workflow/scripts/check_delivery_guides.py" \
  --src <project_dir>/src --caption-layout outputs/<project>/caption-layout.json
```
