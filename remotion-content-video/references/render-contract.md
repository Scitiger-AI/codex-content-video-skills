...
The QC report must include duration, width, height, frame rate, SRT segment count, longest subtitle segment, and `passed`. A missing video, no video stream, invalid dimensions, invalid frame rate, or zero subtitle segments when captions are enabled is a failure.
