...
Use this skill to generate a speech audio file through `https://link.scitiger.cn`. This skill intentionally stops after audio generation and does not request or generate subtitles. Default speech speed is `1.0`; only change it when the user explicitly requests a rate. The bundled `assets/default-reference.mp3` is the portable default voice asset for noncommercial narration only.
...
Use `assets/default-reference.mp3` only to generate noncommercial narration. For commercial, sponsored, client, paid, or monetized use, require `--reference-audio` supplied by the user and confirm that they hold the necessary rights.

Do not train or fine-tune a model on the bundled asset, create another reusable voice or voice variant from it, modify the reference asset, or relicense it. The detailed terms travel with the asset in `assets/NOTICE`.
...
2. Choose a reference audio source: use `--reference-audio` when supplied. Otherwise, use the bundled `assets/default-reference.mp3` only after confirming that the narration is noncommercial. The public API currently requires reference audio and does not expose a `voice_id` field. Never scan a user directory or select a voice at random.
